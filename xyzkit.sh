...
function extract() {
  ...
  if [[ ! -d build/distributions ]]; then
    ...
    echo "Have you run \"./xyzkit.sh build\" yet?"
    echo
    exit
  fi

  if [[ ! -f build/distributions/waltid-xyzkit-$version.tar ]]; then
    echo "The build files do not exist (directory ./build/distributions/waltid-xyzkit-$version.tar)."
    echo "Have you run \"./xyzkit.sh build\" yet?"
    echo
    exit
  fi
  ...
  (
    ...
    if tar xf waltid-xyzkit-$version.tar; then
...
function execute_debug() {
  if [[ -f build/distributions/waltid-xyzkit-$version/bin/waltid-xyzkit ]]; then
    JAVA_OPTS="-Dorg.slf4j.simpleLogger.defaultLogLevel=DEBUG" build/distributions/waltid-xyzkit-$version/bin/waltid-xyzkit "$@"
  else
...
function execute() {
  if [[ -f build/distributions/waltid-xyzkit-$version/bin/waltid-xyzkit ]]; then
    build/distributions/waltid-xyzkit-$version/bin/waltid-xyzkit "$@"
  else
    build_runscript_question
...
function help() {
  ...
  echo
  echo "Use \"execute\" to execute waltid-xyzkit with no arguments. If you don't supply any"
  echo "arguments of {build|build-st|build-docker|build-podman|extract}, waltid-xyzkit will"
